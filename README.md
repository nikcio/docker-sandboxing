...
provider, git, node, dotnet, python, rust, docker, apt, browser automation,
…). Three templates ship: **.NET + Node.js** (NVM + PNPM), **Python + uv**,
and **Rust + cargo** (rustup), all with Git and GitHub CLI preinstalled.
...
`zeldoc`, `git`, `python`; a pure Rust project keeps `opencode-runtime`,
...
- **Templates** (`template-node-dotnet/Dockerfile` → `opencode-node-dotnet:v1`,
  ...
  CPython, uv, Git (+ git-lfs), `gh`, and the same `o` shim; the Rust image
  ships a rustup-managed stable toolchain (clippy + rustfmt + rust-analyzer),
  cargo, the C build toolchain for linking crates, Git (+ git-lfs), `gh`, and
  the same `o` shim. All extend `docker/sandbox-templates:opencode-docker`.
- **Kits** (`kit-node-dotnet/`, `kit-python/`, `kit-rust/`, `kind: sandbox`):
  point at the local template images, set the entrypoint (banner → auto-start
  opencode → login shell on exit), drop a permissive OpenCode config, and
  rebuild the sandbox `AGENTS.md` from the kit's base plus every composed
  mixin's note.
- **`kit-published-node-dotnet/`, `kit-published-python/`,
  `kit-published-rust/`**: same kits, `sandbox.image` pinned to the public
  Docker Hub tags. Release-please bumps versions + tags; keep them in sync
  with `kit-node-dotnet/`, `kit-python/`, and `kit-rust/`.
