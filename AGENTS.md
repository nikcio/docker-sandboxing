...
- **[Branch policies for AI agents](docs/agent-branch-protection.md)** —
  the `main` branch protection that keeps agent work reviewable (PRs,
  required checks, no force pushes). Read when a PR is blocked by branch
  rules or when asked to set them up.
