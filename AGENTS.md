...
- `template/Dockerfile` — the sandbox template image
  ...
  `sbx template load`).
- `kit/` — declarative sandbox kit (`schemaVersion: "2"`, `kind: sandbox`,
  `extends: opencode`). Validate with `sbx kit validate kit/` after every
  change. `kit/files/` holds static files dropped into the sandbox at
  creation (e.g. the Zeldoc opencode config loaded via `OPENCODE_CONFIG`).

Constraints to respect:

- Sandboxes run with a deny-by-default network policy. `permissions.network`
  in `kit/spec.yaml` is the agent's only egress. If a download fails inside a
  sandbox, check `sbx policy log`, add the host, and recreate the sandbox
  (kit changes never apply to running sandboxes).
