...
- `gh` itself is on `PATH` after creation. If the template image does not ship `gh`, the mixin installs it from the official `cli.github.com` apt repository (skipped when `gh` is already available).
