...
build containers against Microsoft Container Registry.
...
```yaml
kits:
  - git+https://github.com/nikcio/docker-sandboxing.git#dir=mixins/mcr   # Microsoft Container Registry egress
```

For local development, point `--kit` at the directory instead:
`sbx run --kit ./mixins/mcr <agent> .`
