...
The proxy injects GitHub credentials for HTTPS Git operations.
...
keychain), scoped per sandbox — **fine-grained PATs only, never in files on disk**. Ask the
user to run on their host, using `$SANDBOX_NAME`
(not the branch or worktree path):
...
`git push -u <fork-remote> <branch>` and open the PR on GitHub from the pushed branch.
