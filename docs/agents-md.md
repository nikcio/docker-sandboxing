...
| Sandbox environment file | Next to the workspace folder, outside your repo | Generated at sandbox start: the agent-context profile (sandbox baseline + one note per mixin) |
...
`filename: AGENTS.md`), and every composed mixin contributes to it via
v3's progressive-disclosure composition — no startup rebuild needed:
...
- the `agents-md` mixin contributes the sandbox baseline
  (`files/home/.sandbox-agents.md` — environment facts plus an index of
  guidance files: network blocks, git auth, workspace mode, persistent
  shell);
- every other mixin contributes its note via its own `agent-context`
  `contentFile` (e.g. `mixins/node/node.md` in the docker-sandboxing
  repo);
- the guidance files the baseline indexes (`~/.sandbox-agents/*.md`)
  ship as plain files in the `agents-md` mixin's image layer.

Because the profile is generated by the runtime:

- it sits **outside your repo** — it never shows up in `git status`;
- it is **recomposed on every sandbox create** — hand-edits are lost;
- per-mixin notes are **staged files, not copies** — the index points at
  the on-disk paths.
