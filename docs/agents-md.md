...
The workload kit owns the profile (`agent-context` with
`filename: AGENTS.md` in every `kit-*/kit-*.yaml` — `filename` is
workload-only; declaring it on a mixin is an error), and every composed
mixin contributes to it via v3's progressive-disclosure composition —
no startup rebuild, no baseline mixin:

- the runtime seeds the profile with its own guidance and a per-kit
  index (a "Kits" section) telling the agent which kit contributed what
  and where to read more on demand;
- the `opencode` mixin contributes the sandbox baseline (environment
  facts: deny-by-default egress, proxy-injected git auth, persistent
  shell, workspace mode);
- every other mixin that ships guidance contributes its note via its own
  `agent-context` `contentFile` (e.g. the `node` mixin's node/pnpm
  note).

Because the profile is generated by the runtime:

- it sits **outside your repo** — it never shows up in `git status`;
- it is **regenerated by the runtime at sandbox create** — hand-edits
  don't survive a recreate;
- per-mixin notes (`contentFile`) are **staged files, not copies** —
  the frontend stages them into the image under
  `/usr/share/sandbox/kit/<stem>/` and the index points the agent at
  those on-disk paths (a mixin using inline `content` instead gets a
  per-kit file beside the profile)
...
3. **Project-specific sandbox notes** (private feeds, egress, agent
   notes): add an `agent-context` `contentFile` to your in-project kit —
